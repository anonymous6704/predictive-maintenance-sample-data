...
```
    ...
    This format describes small extracted subsets from processed survival-frame `.npz` artifacts generated by the OCEAN-MO-CDSF pipeline.
    ...
    | Column | Type | Required | Description |
    | --- | --- | --- | --- |
    | sample_id | string | Yes | Unique mini-sample identifier |
    | dataset | string | Yes | Dataset family |
    | censoring_rate | number | Yes | Scenario censoring rate |
    | unit_id | string | Yes | Anonymized unit identifier |
    | anchor_time | string or number | Yes | Processed anchor value from the source frame |
    | duration | number | Yes | Observed time-to-event or time-to-censoring |
    | event | integer | Yes | `1` observed failure, `0` right-censored |
    | event_type | integer | Yes | Source event code from the processed frame |
    | horizon_1 | number | Yes | Default or project-level first horizon |
    | horizon_2 | number | Yes | Default or project-level second horizon |
    | horizon_3 | number | Yes | Default or project-level third horizon |
    | horizon_4 | number | Yes | Default or project-level fourth horizon |
    | static_feature_1 | number | Yes | First static summary feature |
    | static_feature_2 | number | Yes | Second static summary feature, or `0` if absent |
    | seq_feature_mean_1 | number | Yes | Mean of the first available sequential feature over observed steps |
    | seq_feature_std_1 | number | Yes | Standard deviation of the first available sequential feature over observed steps |
    | seq_feature_last_1 | number | Yes | Last observed value of the first available sequential feature |
    | missing_rate | number | Yes | Missing-value proportion derived from the source frame |
    | padding_rate | number | Yes | Padding proportion derived from the source frame |
    | source_frame | string | Yes | Source `.npz` file name only, not a local path |

    ### Notes

    - These mini files are extracted from processed pipeline frames, not from raw industrial data.
    - `source_frame` must not contain a local Windows path.
    - Unit identifiers are anonymized for public distribution.
    - The mini files are meant for schema inspection and validation, not for reproducing benchmark-scale training or test metrics.
    """
).replace("\n    ", "\n").strip() + "\n"

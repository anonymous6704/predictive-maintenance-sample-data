...
def extract_one(frame_name: str, dataset: str, censoring_rate: float, output_name: str, inspection_lines: list[str]) -> dict[str, Any]:
    ...
    if dataset == "scania":
        inspection_lines.append("- Upstream provenance: SCANIA Component X-derived survival/time-to-event frame; not UCI APS Failure at Scania Trucks.")
    ...
    return {
        ...
        "source_type": "processed_frame_subset",
        "anonymized": True,
        "notes": "Deterministic seed=0 subset extracted from processed survival frames; horizon grid defaulted to [24, 72, 168, 336].",
    }
...
def main() -> int:
    global SOURCE_FRAMES_ROOT
    parser = argparse.ArgumentParser(description="Extract lightweight mini CSV samples from processed .npz survival frames.")
    parser.add_argument("--frames-root", default=str(SOURCE_FRAMES_ROOT))
    args = parser.parse_args()
    SOURCE_FRAMES_ROOT = Path(args.frames_root)
